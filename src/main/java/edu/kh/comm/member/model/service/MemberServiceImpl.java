...
@Service // 비지니스 로직(데이터 가공, DB연결)을 처리하는 클래스 임을 명시 + bean 등록
public class MemberServiceImpl implements MemberService{
	 ...
	 * 어쩔수 없이 Connection을 Service 에서 얻어올 수 밖에 없었음
	 */
	...
	public Member login(Member inputMember) {
		...
		// sha방식 암호화 
		// A회원/ 비밀번호1234 => 암호화 : abcd 
		// B회원/ 비밀번호1234 => 암호화 : abcd(암호화시 변경된 내용이 같음)
		
		// Bcrypt 암호화 : 암호화 하기전에 Salt 를 추가하여 변형된 상태로 암호화를 진행
		// A 회원 / 비밀번호 1234 => 암호화 : abcd
		// B 회원 / 비밀번호 1234 => 암호화 : @afg
		...
		logger.debug(inputMember.getMemberPw()+"/"+bcrypt.encode(inputMember.
				 getMemberPw()));
		/*
		 * logger.debug(inputMember.getMemberPw()+"/"+bcrypt.encode(inputMember.
		 * getMemberPw()));
		...
		if(loginMember != null) { // 일치하는 이메일을 가진 회원 정보가 있을 경우
			//					입력된 비밀번호 (평문) vs 조회된 비밀번호 (암호화) => 같으면 true
			if( bcrypt.matches(inputMember.getMemberPw(),loginMember.getMemberPw())) {
				
				loginMember.setMemberPw(null); // Session에 비밀번호가 올라가지않도록 null 처리
				
			} else { //비밀전호가 일치하지 않을 경우
				loginMember = null;
			}
