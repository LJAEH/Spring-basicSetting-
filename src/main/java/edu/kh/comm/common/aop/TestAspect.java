...
@Aspect    // 공통 관심사가 작성된 클래스임을 명시한다. (모듈화된 특정 흐름 사이에 끼여서 수행할 코드)
		   // => 해당 어노테이션이 작성된 클래스에는 
...
public class TestAspect {
	...
	// edu.kh.comm.member 패키지 아래의 Impl롷 끝나는 클래스의 모든 메서드(매개변수 관계없음)
	//@Before("memberPointcut()")
	@Before("CommonPointcut.implPointcut()")
	public void start() {
		logger.info("-------------------- MemberService Start -----------------------");
	}
	//@After("memberPointcut()")
	@After("CommonPointcut.implPointcut()")
	public void end() {
		logger.info("-------------------- MemberService End -----------------------");
	}
	// Pointcut을 작성해놓은 메서드
	// => Pointcut의 패턴이 작성되는 부분에 memberPointcout 메서드 이름을 작성하면
