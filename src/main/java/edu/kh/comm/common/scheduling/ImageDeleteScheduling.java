...
public class ImageDeleteScheduling {
	...
	@Autowired
	private ServletContext application; // application scope객체 => 서버 폴더 경로 얻어오기에 사용할 거임
	...
	public void serverImageDelete() {
		...
		// 3) 두 목록을 비교해서 일치하지 않는 이미지파일 삭제
		// (DB에는 없는데 서버에만 있는 파일들 삭제)
		if(!serverList.isEmpty()) { // 서버에 이미지 파일이 있을때 비교/삭제 진행
			// =>DB 상 경로 /resources/images/board/sample2.jpg
			// =>서버 상 경로\resources\images\board\sample2.jpg
			for(File serverImage : serverList ) {
				String name = "/resources/images/board/" + serverImage.getName(); // 파일명만 얻어오기		
				
				if(dbList.indexOf(name)==-1) { // List에 value와 같은 값이 있으면 인덱스반환/ 없으면 -1 반환
					// dbList에는 없는 serverList에는 있는 경우
